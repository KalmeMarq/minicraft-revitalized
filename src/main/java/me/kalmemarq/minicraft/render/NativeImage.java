...
public class NativeImage implements AutoCloseable {
	...
	public NativeImage(int width, int height, PixelFormat format) {
		...
		this.pointer = MemoryUtil.nmemAlloc(this.width * this.height * (long) format.channelCount);
	}
	...
	public static NativeImage read(Framebuffer framebuffer) {
		NativeImage image = new NativeImage(framebuffer.getWidth(), framebuffer.getHeight(), PixelFormat.RGB);
		image.loadFromFramebuffer(framebuffer);
		return image;
	}

	public void loadFromTexture(Texture texture) {
		GL45.glGetTextureImage(texture.getHandle(), 0, GL33.GL_RGBA, GL33.GL_UNSIGNED_BYTE, this.width * this.height * 4, this.pointer);
	}

	public void loadFromFramebuffer(Framebuffer framebuffer) {
		GL45.glGetTextureImage(framebuffer.getColorAttachment(), 0, GL33.GL_RGB, GL33.GL_UNSIGNED_BYTE, this.width * this.height * 3, this.pointer);
		this.flip(Mirroring.VERTICAL);
	}
